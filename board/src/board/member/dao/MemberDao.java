...
public class MemberDao {
	...
	public MemberVo getMemberLoginInfo(String id) {
		...
		MemberVo vo = null;
		try {
			pstmt = con.prepareStatement("select id, pwd from member where id=? and del_fl=0");
	...
	public MemberVo getMemberInfo(String id) {
		...
		MemberVo vo = null;
		try {
			pstmt = con.prepareStatement("select id,name from member where id=?");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while(rs.next()) {
				vo = new MemberVo();
	...
	public int leaveMember(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("update member set del_fl=? where id=?");
